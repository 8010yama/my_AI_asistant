...
def get_address_from_gps(lat: float | None, lon: float | None) -> str:
...
def _build_wav_response(chunks: list[bytes], headers: dict | None = None) -> Response:
    output = io.BytesIO()
    ...
    return Response(content=output.getvalue(), media_type="audio/wav", headers=headers or {})

def _reply_to_response(reply: str) -> Response:
    audio = text_to_speech(reply)
    if not audio:
        return Response(status_code=204)
    return _build_wav_response([audio])
...
async def alia(payload: TextPayload, x_api_key: str = Header(...)):
    ...
    if memo_type:
        ...
        return _reply_to_response(build_memo_reply(success, memo_type))
    ...
    if calendar_type == "add":
        ...
        return _reply_to_response(build_calendar_add_reply(success, title, start_dt, end_dt, has_time))
        ...
        if info:
            ...
            return _reply_to_response(build_calendar_update_reply(success, title, start_hour, new_end_dt))
        return _reply_to_response("更新する予定の時間が読み取れませんでした。")
        ...
        return _reply_to_response(build_calendar_read_reply(events, target_date))
    ...
    if use_train:
        ...
        start_station = from_match.group(1).split()[-1] if from_match else get_nearest_station(address)
        if start_station:
        ...
        station = from_match.group(1).split()[-1] if from_match else get_nearest_station(address)
        if station:
    ...
    return _build_wav_response(audio_chunks, headers)
